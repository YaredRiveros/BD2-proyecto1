...
const int globalDepth = 10; //Profundidad global
const int maxRecords = 100; //Cantidad máxima de registros por bucket
const int TotalIndices = (long int)pow(2, globalDepth); //Cantidad de indices en index.bin
...
class ExtendibleHash{
    ...
    bool validarId(Record record){
        ...
        Indice index;
        int hash = hash_fn(record.id)%TotalIndices;
        ...
        if(existe == false){
            ...
            //3. Si está lleno, revisar que el depth local sea menor al global
                ...
                if(bucket.localDepth<globalDepth){
                    ...
                    for(int i=0;i<bucket.nRecords;i++){
                        //Aqui si entra
                        hashC2 = hash_fn(bucket.records[i].id)%TotalIndices;
    ...
    Record searchRecord(int id,int &count){
        try{
            //1. Hallo el hash del id
            long int numIndices = TotalIndices;
            int hash = hash_fn(id) % numIndices;
    ...
    void LoadCsv(const string& FileName){
        ...
        while(getline(file,line)){
            ...
            insertRecord(record,count);
            total += count;

        }
        cout << "Total de accesos a memoria secundaria: " << total << endl;
        file.close();
    }
